...
workon zuppe  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development zuppe npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop zuppe
    docker rm zuppe
    docker run --name zuppe -d --env-file /home/ubuntu/zuppe.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/zuppe:/dkdata \
        zuppe start_web.sh
}
...
echo_green "Welcome to zuppe's dev env"
